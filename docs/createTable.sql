...
(
    ...
    CODE   VARCHAR(20)  NOT NULL UNIQUE COMMENT '대리점 코드 (사업자등록번호 등, 고유값)',
...
CREATE TABLE USR_ROLE_LOG
(
    ID         BIGINT PRIMARY KEY COMMENT '로그 고유 ID (PK)',
    USR_ID     BIGINT      NOT NULL COMMENT '권한이 변경된 사용자 ID (FK)',
    CHANGER_ID BIGINT      NOT NULL COMMENT '권한을 변경한 관리자 ID (FK)',
    OLD_ROLE   VARCHAR(20) NOT NULL COMMENT '변경 전 권한',
    NEW_ROLE   VARCHAR(20) NOT NULL COMMENT '변경 후 권한',
    REG_DT     TIMESTAMP DEFAULT CURRENT_TIMESTAMP() COMMENT '등록일시',
    FOREIGN KEY (USR_ID) REFERENCES USR (ID),
    FOREIGN KEY (CHANGER_ID) REFERENCES USR (ID)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci COMMENT '사용자 권한 변경 이력을 기록하는 테이블';
